...
namespace Elysia.Components
{
    ...
    {
        public delegate void OnLinkClickedEvent(int index, TMP_LinkInfo linkInfo);
        ...
        {
            int index = TMP_TextUtilities.FindIntersectingLink(_text, eventData.position, App.Scene.UI.Camera);
            if (index < 0)
            {
                return;
            }
