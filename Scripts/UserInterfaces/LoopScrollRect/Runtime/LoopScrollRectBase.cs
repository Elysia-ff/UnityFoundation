// source from https://github.com/qiankanglai/LoopScrollRect v1.1.3

// Changes
//  - MultiPrefab 지원 삭제 (쓸 일 없음)
//  - Prefab 지정 방식 변경 (Guid 사용)
//  - Pooling 적용
//  - Transform 으로 캐싱하던 것을 LoopScrollElement 로 변경
//  - LateUpdate 등에서 매번 계산하던 것을 캐시 (eg. LoopScrollElement.PreferredHeight)

// How to use
//  - Hierarchy 오른클릭 -> UI -> Loop Horizontal/Vertical Scroll Rect 로 생성하는 것을 권장
//  - LoopScrollElement 를 상속받는 클래스 생성 후 ElementPrefab 으로 지정
//  | - element 의 크기는 LayoutElement 에서 지정해야 함
//  | - element 의 크기는 생성 될때 초기값으로 캐싱됨 동적으로 변경되어야 하는 경우 AutoLayout 을 사용하지 말고
//  | - SetPreferredWidth 혹은 SetPreferredHeight 를 호출해야 함
//  | - inspector 에서 캐싱된 값과 실제 사이즈가 동일한 지 확인 가능
//  | - element 의 active 는 CanvasGroup.alpha 로 컨트롤 됨
//  | - LoopScrollRectBase::SetContext(object) 로 설정한 context 와 elementIndex 가 UpdateUI 에 전달됨
//  - 스크롤 방향과 ReverseDirection 여부가 Content 의 min/max anchors, pivot 과 일치해야 함 (see. Assert in Awake())
//  - totalCount 의 초기값은 0으로 유지
//  | - 초기값을 지정하면 생성과 동시에 element 가 생성되지만 UI 의 Initialize 보다 호출 시점이 빠르므로 이슈 있음
//  | - 어차피 UI 의 Initialize 등에서 데이터를 받아와서 새로 생성해야 하므로 0 외의 값은 사용하지 않음
//  - element 의 크기가 서로 다르고 ScrollBar 를 사용하는 경우 sizeProvider 에 element 의 크기를 계산하는 함수를 등록해야 함
//  - LateUpdate 가 호출되므로 UI 가 종료될 때 enable 이 같이 관리되어야 함

/* example
InputField 에서 입력한 string 을 scroll view 에 표시하는 ui
...
protected override void Initialize()
{
    ...
    _scrollRect.SetSizeProvider(GetElementSize);
}

private Vector2 GetElementSize(int itemCount)
{
    float height = 0f;

    for (int i = 0; i < itemCount; i++)
    {
        // sizeProvider 는 호출 횟수가 잦으므로 실제 코드에선 캐시되어야 함
        height += ChatHeights[i];
    }
...
{
    ChatElement prefab = App.ResourceHolder.GetUI<ChatElement>(_scrollRect.ElementPrefabGuid);

    string text = $"[{_chatList.Count}] {message}";
    _chatList.Add(text);

    // element 의 height 가 text 길이에 따라 변경되므로 각 text 별 크기를 캐시
    float height = prefab.GetPreferredHeight(580f, text);
    _chatHeights.Add(height);
...
****** ChatElement.cs ******
public override void UpdateUI(object context, int index)
{
    string message = ChatUI.ChatList[index];
    _label.SetText(message);

    // text 크기에 맞는 사이즈 지정
    float height = ChatUI.ChatHeights[index];
    SetPreferredHeight(height);
}

// 주어진 width 와 text 에 대한 height 를 반환
public float GetPreferredHeight(float width, string text)
{
    return _label.GetPreferredValues(text, width, 0f).y;
}
*/
...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        /// </summary>
        [SerializeField] private ComponentReferenceT<LoopScrollElement> _elementPrefab;
        public StringID ElementPrefab => _elementPrefab.Key;

        private readonly List<LoopScrollElement> _elements = new List<LoopScrollElement>();
        private readonly Stack<LoopScrollElement> _pool = new Stack<LoopScrollElement>();
        ...
        public delegate Vector2 SizeProviderDelegate(int itemCount);
        /// <summary>
        /// [Optional] Helper for accurate size so we can achieve better scrolling.
        /// </summary>
        private SizeProviderDelegate _sizeProvider = null;
        ...
        protected abstract float GetSize(LoopScrollElement item, bool includeSpacing = true);
        ...
        public void SetSizeProvider(SizeProviderDelegate sizeProvider)
        {
            _sizeProvider = sizeProvider;
        }

        protected LoopScrollElement CreateElement()
        {
            LoopScrollElement element;
            if (_pool.Count > 0)
            {
                element = _pool.Pop();
            }
            ...
            {
                LoopScrollElement prefab = App.ResourceHolder.GetUI<LoopScrollElement>(_elementPrefab.Key);
                element = Instantiate(prefab, m_Content, false);
                element.Initialize();
            }
        ...
        protected void ReturnElement(int index)
        {
            ReturnElement(GetElement(index));
        }

        protected void ReturnElement(LoopScrollElement element)
        {
            element.Deactivate();
        ...
        protected LoopScrollElement GetElement(int index)
        {
        ...
        protected void RearrangeElement(LoopScrollElement element, int index)
        {
            _elements.Remove(element);
        ...
        public int GetLastItem(out float offset)
        {
            ...
            if (itemTypeEnd > itemTypeStart)
            {
                int totalChildCount = GetElementCount();
                float size = GetSize(GetElement(totalChildCount - idx - 1), false);
                while (size + offset <= 0 && itemTypeStart < itemTypeEnd - idx - contentConstraintCount)
                {
                    offset += size;
                    idx += contentConstraintCount;
                    size = GetSize(GetElement(totalChildCount - idx - 1));
                }
            }
            offset = -offset;
        ...
        public void ScrollToCellWithinTime(int index, float time, float offset = 0, ScrollMode mode = ScrollMode.ToStart)
        {
            ...
            else
            {
                ...
                {
                    dist = GetDimension(_sizeProvider(currentFirst) - _sizeProvider(index)) + contentSpacing * (CurrentLine - TargetLine);
                    dist -= currentOffset;
                }
                else
                {
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < GetElementCount(); i++)
                {
                    ...
                    {
                        GetElement(i).UpdateUI(_context, itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int endItem = 0, float contentOffset = 0)
        {
            ...
            ReturnToTempPool(!reverseDirection, GetElementCount());
        ...
        protected float NewItemAtStart(bool includeSpacing = true)
        {
            ...
            {
                itemTypeStart--;
                LoopScrollElement newItem = GetFromTempPool(itemTypeStart);
                RearrangeElement(newItem, deletedItemTypeStart);
        ...
        protected float DeleteItemAtStart()
        {
            ...
            int availableChilds = GetElementCount() - deletedItemTypeStart - deletedItemTypeEnd;
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                LoopScrollElement oldItem = GetElement(deletedItemTypeStart);
        ...
        protected float NewItemAtEnd(bool includeSpacing = true)
        {
            ...
            {
                LoopScrollElement newItem = GetFromTempPool(itemTypeEnd);
                RearrangeElement(newItem, GetElementCount() - deletedItemTypeEnd - 1);
                size = Mathf.Max(GetSize(newItem, includeSpacing), size);
                itemTypeEnd++;
                if (totalCount >= 0 && itemTypeEnd >= totalCount)
                {
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                LoopScrollElement oldItem = GetElement(GetElementCount() - deletedItemTypeEnd - 1);
                size = Mathf.Max(GetSize(oldItem), size);
        ...
        private LoopScrollElement GetFromTempPool(int itemIdx)
        {
            LoopScrollElement nextItem = null;
            if (deletedItemTypeStart > 0)
            {
                deletedItemTypeStart--;
                nextItem = GetElement(0);
                RearrangeElement(nextItem, itemIdx - itemTypeStart + deletedItemTypeStart);
            }
            else if (deletedItemTypeEnd > 0)
            {
                deletedItemTypeEnd--;
                nextItem = GetElement(GetElementCount() - 1);
                RearrangeElement(nextItem, itemIdx - itemTypeStart + deletedItemTypeStart);
            }
            else
            {
                nextItem = CreateElement();
            }

            nextItem.UpdateUI(_context, itemIdx);
        ...
        //==========LoopScrollRect==========
        public void GetHorizontalOffsetAndSize(out float totalSize, out float offset)
        {
            if (_sizeProvider != null)
            {
                totalSize = _sizeProvider(TotalLines).x + contentSpacing * (TotalLines - 1);
                offset = m_ContentBounds.min.x - _sizeProvider(StartLine).x - contentSpacing * StartLine;
            }
            else
            {
        ...
        public void GetVerticalOffsetAndSize(out float totalSize, out float offset)
        {
            if (_sizeProvider != null)
            {
                totalSize = _sizeProvider(TotalLines).y + contentSpacing * (TotalLines - 1);
                offset = m_ContentBounds.max.y + _sizeProvider(StartLine).y + contentSpacing * StartLine;
            }
            else
            {
        ...
        private void UpdateScrollbars(Vector2 offset)
        {
            if (m_HorizontalScrollbar)
            {
                ...
                {
                    float totalSize, _;
                    GetHorizontalOffsetAndSize(out totalSize, out _);
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                ...
                if (totalCount > 0 && itemTypeEnd > itemTypeStart)
                {
                    ...
                    GetHorizontalOffsetAndSize(out totalSize, out offset);
        ...
        private Bounds GetBounds4Item(int index)
        {
            ...
            if (offset < 0 || offset >= GetElementCount())
                return new Bounds();

            var rt = GetElement(offset).RectTransform;
            if (rt == null)
                return new Bounds();
